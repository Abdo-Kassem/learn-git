...
@section('pageTitle') 
    <i class="fa fa-edit"></i> {{ trans('backend.edit') }} {{ trans('backend.areas') }} 
@endsection
    ...
    <div class="box">
        ...
        <div class="box-header with-border">
            ...
            <div class="button-page-header" style="margin-top:5px">
                <a class="btn btn-block btn-warning" href="{{ route('areas.index') }}">
                <i class="fa fa-reply fa-fw fa-lg"></i> {{ trans('backend.back') }}</a>
            </div>
        </div>

        <div class="box-body">      
            <form id="myForm" action="{{ route('areas.update' , $area->id) }}" method="POST" class="userForm" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name_ar" id="name_ar" class="form-control {{ $errors->has('name_ar') ? 'is-invalid' : '' }}" value="{{ $area->name_ar }}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name_fr" id="name_fr" class="form-control {{ $errors->has('name_fr') ? 'is-invalid' : '' }}" value="{{ $area->name_fr }}">
                            ...
                            <select name="city_id" id="city_id" class="form-control select2">
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" {{ $area->city_id == $city->id ? 'selected' : '' }}>
                                        {{ app()->getLocale() == 'ar' ? $city->name_ar : $city->name_fr }}
                                    </option>
                                @endforeach
                            </select>
